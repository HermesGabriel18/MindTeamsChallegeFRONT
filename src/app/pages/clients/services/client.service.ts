import { Injectable } from '@angular/core';
import { ClientsEndpoints } from '@core/models';
import { ApiService, UtilsService } from '@core/utils';
import { PaginatorLinks, PaginatorMeta } from '@shared/models';
import { Observable } from 'rxjs';
import { map } from 'rxjs/operators';
import { Client } from '../models';

@Injectable({
  providedIn: 'root',
})
export class ClientService {
  private _links: PaginatorLinks;
  private _meta: PaginatorMeta;
  constructor(
    private _apiService: ApiService,
    private _utilsService: UtilsService
  ) {}

  set links(links: PaginatorLinks) {
    this._links = links;
  }

  set meta(meta: PaginatorMeta) {
    this._meta = meta;
  }

  get links(): PaginatorLinks {
    return this._links;
  }

  get meta(): PaginatorMeta {
    return this._meta;
  }

  getAllClients({
    pageIndex = null,
    pageSize = null,
    dontPaginate = null,
    filters = null,
  } = {}): Observable<Client[]> {
    return this._apiService
      .get(
        ClientsEndpoints.clients,
        this._utilsService.buildHttpOptions({
          pageIndex,
          pageSize,
          dontPaginate,
          filters,
        })
      )
      .pipe(
        map((response) => {
          this.links = response.links || null;
          this.meta = response.meta || null;
          return response.data;
        })
      );
  }

  getClientById(id: number, filters = null): Observable<Client> {
    return this._apiService
      .get(
        ClientsEndpoints.clientsWithId.replace(':id', String(id)),
        this._utilsService.buildHttpOptions({
          filters,
        })
      )
      .pipe(map((response) => response.data));
  }

  createClient(data: Client): Observable<Client> {
    return this._apiService
      .post(ClientsEndpoints.clients, data)
      .pipe(map((response) => response.data));
  }

  updateClient(data: Client): Observable<Client> {
    return this._apiService
      .put(ClientsEndpoints.clientsWithId.replace(':id', String(data.id)), data)
      .pipe(map((response) => response.data));
  }

  deleteClient(client_id: string): any {
    return this._apiService.delete(
      ClientsEndpoints.clientsWithId.replace(':id', String(client_id))
    );
  }
}
